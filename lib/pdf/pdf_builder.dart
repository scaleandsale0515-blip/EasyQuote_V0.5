...
import 'pdf_template_builders.dart';
...
class DocumentPdfBuilder {
  static Future<Uint8List> build({
    ...
    // Use the stored active template if not explicitly overridden.
    final int tIdx = templateIndex >= 0
        ? templateIndex
        : _getActiveTemplateIndex();

    // Template 0 (Classic Dark) uses the original builder; others use the
    // new template builders.
    if (tIdx > 0 && tIdx < PdfTemplate.all.length) {
      return PdfTemplateBuilders.build(
        profile: profile,
        client: client,
        doc: doc,
        template: PdfTemplate.all[tIdx],
      );
    }

    // Default: original Classic Dark builder below.
    return _buildClassicDark(profile: profile, client: client, doc: doc);
  }

  static int _getActiveTemplateIndex() {
    try {
      return LocalDB.instance.getActivePdfTemplate();
    } catch (_) {
      return 0;
    }
  }

  static Future<Uint8List> _buildClassicDark({
    required CompanyProfile profile,
    required Client? client,
    required QuoteDoc doc,
  }) async {
    ...
    final safeClient = client ?? Client(id: '', companyName: 'Demo Client',
    ...
    final intro = doc.introText.isNotEmpty
        ...
        : 'We are pleased to submit detailed ${isQuotation ? 'Quotation' : 'Invoice'} for '
            '${safeClient.companyName.isEmpty ? 'your company' : safeClient.companyName}'
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (context) => [
          ...
          // ---- Terms & Conditions: only when the toggle is on, and only
          // when the preset actually has clauses. Always starts on its own
          // fresh page when shown. ----
          if (doc.includeTerms && ts.clauses.isNotEmpty) ...[
            pw.NewPage(),
            _buildTermsHeading(fonts),
            _buildTermsList(ts, fonts),
            ...
            children: [
              pw.RichText(
                text: pw.TextSpan(children: [
                  ...
                  pw.TextSpan(
                      text: safeClient.companyName.isEmpty ? '—' : safeClient.companyName,
                      style: pw.TextStyle(font: fonts.regular, fontSize: 9.5)),
                ]),
              ),
              if (safeClient.contactPerson.isNotEmpty)
                pw.Text('Kind Attn.: ${safeClient.contactPerson}', style: pw.TextStyle(font: fonts.regular, fontSize: 9.5)),
              if (safeClient.phone.isNotEmpty)
                pw.Text('Mo: ${safeClient.phone}', style: pw.TextStyle(font: fonts.regular, fontSize: 9.5)),
              if (safeClient.email.isNotEmpty)
                pw.Text('Email: ${safeClient.email}', style: pw.TextStyle(font: fonts.regular, fontSize: 9.5)),
            ],
          ),
        ),
  ...
  static pw.Widget _buildTermsList(TermsPreset ts, PdfFonts fonts) {
    final mainStyle = pw.TextStyle(font: fonts.regular, fontSize: 9);
    final numStyle = pw.TextStyle(font: fonts.bold, fontSize: 9);

    return pw.Column(
      crossAxisAlignment: pw.CrossAxisAlignment.start,
      children: ts.clauses.asMap().entries.map((entry) {
        final n = entry.key + 1;
        ...
        return pw.Padding(
          ...
          child: pw.Row(
            crossAxisAlignment: pw.CrossAxisAlignment.start,
            children: [
              pw.SizedBox(width: 18, child: pw.Text('$n.', style: numStyle)),
              pw.Expanded(child: pw.Text(text, style: mainStyle)),
            ],
          ),
        );
      }).toList(),
